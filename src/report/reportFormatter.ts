...
export function formatTerminalDashboard(snapshot: ReportSnapshot): string {
  ...
  const lines = [
    ...
    `Gross unrealized:   ${formatUsd(p.grossUnrealizedPnlUsd)}`,
    `Est. net unrealized: ${formatUsd(p.estimatedNetUnrealizedPnlUsd)}`,
    `Total fees paid:    ${formatUsd(p.totalFeesPaidUsd)}`,
    `Est. exit fees:     ${formatUsd(p.estimatedFutureExitFeesUsd)}`,
